...
def calc_fourier_map(
    ...
    """Compute a 3D Fourier map given a ScatteringData object :param
    data: a ScatteringData object with observed data, either q
    DiffractionDataSingleCrystal or PowderPatternDiffraction after
    extraction of intensities (profile fitting) :param map_type: either
    "obs" (the default), "diff" (difference) or "calc" :param sharpen:
    if True, normalise the structure factor Fourier coefficients by the
    average atomic scattering factor to sharpen the Fourier maps.

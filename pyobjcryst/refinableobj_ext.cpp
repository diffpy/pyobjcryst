                 ...
                 public wrapper<RefinableObj>
{
    ...
    void AddSubRefObj(RefinableObj &so)
    { RefinableObj::AddSubRefObj(so); }

    void RemoveSubRefObj(RefinableObj &so)
    { RefinableObj::RemoveSubRefObj(so); }
...
BOOST_PYTHON_MODULE(_refinableobj)
{
    ...
    class_<RefinableObjWrap, boost::noncopyable>("RefinableObj")
        ...
        // This one copies all RefinablePars from one RefinableObj to another.
        // There is a complex set of instructions for how to handle the memory
        // in this case. Since all methods are exposed that manage this, I'll
        // not assign a call policy.
        //.def("AddPar", 
        //        (void (RefinableObj::*)(const RefinablePar&)) 
        //        &RefinableObj::AddPar)
        // FIXME - need call policy here
        .def("AddPar", 
                (void (RefinableObj::*)(RefinablePar*)) 
                &RefinableObj::AddPar)
        .def("RemovePar", &RefinableObj::RemovePar)
        ...
        .def("GetParamSet", (CrystVector<float>& (RefinableObj::*)
                (const unsigned long)) &RefinableObj::GetParamSet,
                return_internal_reference<>())
        ...
        // It appears as if the roll of the design of the ObjRegistry class is
        // changing, so I'm putting off wrapping that object.
        ...
        // This causes a segmentation fault in a bare RefinableObj
